...
@register_criterion('label_smoothed_immt_cross_entropy')
class LabelSmoothedImmtCrossEntropyCriterion(FairseqCriterion):

    def __init__(self, task, sentence_avg, label_smoothing, loss1_coeff, loss2_coeff):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        net_output, kd_loss1, kd_loss2 = model(**sample['net_input'])  # <fix>
        loss, nll_loss = self.compute_loss(model, net_output, sample, kd_loss1, kd_loss2, reduce=reduce)  # <fix>
    ...
    def compute_loss(self, model, net_output, sample, kd_loss1, kd_loss2, reduce=True):
        ...
        loss = loss + self.loss1_coeff * kd_loss1 + self.loss2_coeff * kd_loss2  # <fix>
        return loss, nll_loss
